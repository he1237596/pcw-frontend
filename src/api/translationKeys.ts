...
export const getProjectKeys = async (
  projectId: number,
  { current: page, pageSize: limit, search }: PaginationProps,
) => {
  try {
    const response = await apiClient.get(`/projects/${projectId}/keys`, {
      limit,
      page,
...
export const updateTranslationKeyByLangId = async (
  langId: string,
  params: any,
) => {
  try {
    const response = await apiClient.put(
      `/projects/transaltions/${langId}`,
      params,
    );
    return response;
  } catch (error) {
