...
const TranslationKeyForm = ({ projectId, translationKey, onSuccess, projectList }: any) => {
  ...
  const handleSubmit = async (values: any) => {
    try {
      const { keyName, en, zh_cn, zh_hant, es, fr, it, ja, kr, description} = values;
      const params = {
        keyName,
        description,
        translations: [{
      ...
      if (translationKey) {
        const res = await updateTranslationKey(projectId, translationKey.id, params);
        if (res.code === 200) {
