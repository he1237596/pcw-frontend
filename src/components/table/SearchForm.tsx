...
const SearchForm = forwardRef<SearchFormRef, SearchFormProps>(({ form, items, onSearch, onReset, initialValues, extraButtons }, ref) => {
  ...
  return (
    <div ref={wrapperRef}>
      <Form form={form} initialValues={initialValues} layout="inline" onFinish={onSearch}>
        {items.map(({ name, label, type, placeholder, options, readOnly = false, disabled = false, allowClear = true, ...other }) => (
          <Form.Item key={name} name={name} label={label} style={{ minWidth: 160 }}>
            {type === 'input' ? (
              <Input placeholder={placeholder} readOnly={readOnly} disabled={disabled} allowClear={allowClear} {...other} />
            ) : (
              readOnly?<Select placeholder={placeholder} options={options} open={!readOnly} onClick={(e) => e.preventDefault()} />:
              <Select placeholder={placeholder} options={options} disabled ={disabled} allowClear={allowClear} {...other} />
            )}
          </Form.Item>
        ))}
        <Form.Item>
